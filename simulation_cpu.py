...
def make_same_level_kernels(state_genotype, n_layers, around_start, above_start):
    kernels = np.zeros((n_layers, 3, 3))
    for l in range(n_layers):
        kernels[l] = state_genotype[l][around_start:above_start].reshape((3,3))
    return kernels

def make_below_kernels(state_genotype, n_layers, around_start):
    kernels = np.zeros((n_layers-1, 2,2))
    for l in range(n_layers-1):
        kernels[l] = state_genotype[l+1][0:around_start].reshape((2,2))
    return kernels

def run_simulation(growth_genotype, state_genotype, phenotype, num_layers, base_layer, around_start, above_start, use_growth, activation, above_map, below_map):
    """Run HNCA simulation"""

    # phenotype = list of numpy arrays, each np array is a layer
    n_steps = len(phenotype[0])
    n_layers = len(phenotype)

    # 3x3 kernels to convolve across same level
    same_level_kernels = make_same_level_kernels(state_genotype, num_layers, around_start, above_start)
    # 2x2 kernels to convolve across the layer below
    below_kernels = make_below_kernels(state_genotype, num_layers, around_start)

    for t in range(1, n_steps):
        for l in range(n_layers):
            # Convolve for the same-level neighbors
            phenotype[l][t] = convolve2d(phenotype[l][t-1], same_level_kernels[l], mode='same')

            # Add up/down weights according to above and below maps
            if l < n_layers-1:
                phenotype[l][t] += np.repeat(np.repeat(phenotype[l+1][t-1], 2, axis=0), 2, axis=1) * state_genotype[l, above_start]

            # Add down cells multiplied by down weights
            if l > 0 and n_layers > 1:
                phenotype[l][t] += convolve2d(phenotype[l-1][t-1], below_kernels[l-1], mode='valid')[::2, ::2]

            # Activate sigmoid and only include values > 0 
            mask = phenotype[l][t] > 0
            phenotype[l][t] = activate_sigmoid(phenotype[l][t]) * mask
